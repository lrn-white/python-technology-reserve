...
class AprNode(object):
    ...
    def __hash__(self):
        return hash("_".join(sorted([str(itm) for itm in list(self.s)])))

    def __eq__(self, other):
        if "_".join(sorted([str(itm) for itm in list(self.s)])) == "_".join(
...
class AprBlk():
    ...
    def getFreqItem(self, thd=1, hd=1):
        freq_items = []
...
class AssctAnaClass():
    ...
    def get_freq(self, thd=1, hd=1):
        return self.apr_blk.getFreqItem(thd=thd, hd=hd)
